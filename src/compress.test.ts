...
it.each`
  ...
  ${'gzip'}        | ${'Hello World!'} | ${['H4sIAAAAAAAAA/NIzcnJVwjPL8pJUQQAoxwpHAwAAAA=', 'H4sIAAAAAAAAE/NIzcnJVwjPL8pJUQQAoxwpHAwAAAA=']}
...
`('should compress a regular string with $encoding', async ({ encoding, input, output }) => {
  expect(output).to.include(await compress(input, encoding));
});
